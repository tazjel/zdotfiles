...
class Wiki2Plain:
    ...
    def unwiki(self, wiki):
        ...
        """
        wiki = 
re.sub(r'(?i)\{\{IPA(\-[^\|\{\}]+)*?\|([^\|\{\}]+)(\|[^\{\}]+)*?\}\}', lambda m: 
m.group(2), wiki)
        wiki = 
re.sub(r'(?i)\{\{Lang(\-[^\|\{\}]+)*?\|([^\|\{\}]+)(\|[^\{\}]+)*?\}\}', lambda 
m: m.group(2), wiki)
        wiki = re.sub(r'\{\{[^\{\}]+\}\}', '', wiki)
...
wiki)
    ...
    def image(self):
        ...
        """
        # match = 
re.search(r'(?i)\|?\s*(image|img|image_flag)\s*=\s*(<!--.*-->)?\s*([^\\/:*?<>"|%
]+\.[^\\/:*?<>"|%]{3,4})', self.wiki)
        match = re.search(r'(?i)([^\\/:*?<>"|% =]+)\.(gif|jpg|jpeg|png|bmp)', 
self.wiki)
...
'''Uruguay''' is a country in [[South America]]. The language spoken there is 
Spanish. Its [[capital (city)|capital]] and largest [[city]] is [[Montevideo]]. 
Uruguay is bordered by two large neighbors, [[Brazil]] and [[Argentina]]. The 
only country in South America that is smaller than Uruguay is [[Suriname]].
