...
def wproc():
    ...
    #for line in open('applications/sysadmin/cache/conflict.tmp','r').readlines():
       # items=line.split()
        #items[0]=A(items[0],_href=URL('wfinger/'+items[0]))
        #items[1]=DIV(A(items[1]+' ',_href=URL('man/'+items[1])),
                      #' '+' '.join(items[1:]))
        #items.insert(2,A('wkill',_href=URL('wkill/'+items[1])))
       # table.components.append(TR(*items[:]))
    zsnip =  open('applications/sysadmin/cache/conflict.tmp','r').read()

    
    return dict(table=table, zsnip=zsnip)
...
def markup(text):
 ...
 #   os.system('find /media/ahmed/Transcend/backup/tst/Projects \( -name "*.txt" \) -type f -#print  xargs grep -Enis "conflict is" > applications/sysadmin/cache/conflict.txt')

#       os.system('grep -IR "conflict" /media/ahmed/Transcend/backup/tst/Projects > applications/sysadmin/cache/conflict.txt')
#     os.system('ls /media/ahmed/Transcend/backup/tst/Projects > applications/sysadmin/cache/conflict.txt')
#     os.system('ps auxw > applications/sysadmin/cache/conflict.tmp')


    #os.system('sudo ag -rIn "conflict" /media/ahmed/Transcend/backup/tst/Projects >> applications/sysadmin/cache/conflict.tmp')
    
    #os.system('ag "conflict" /media/ahmed/Transcend/backup/tst/Projects > applications/sysadmin/cache/conflict.tmp')
...
def proc():
    ...
    for line in open('applications/sysadmin/cache/conflict_is.tmp','r').readlines()[1:]:
        items=line.split()
